...
tt_tissue <- read.delim(file.path(out_dir, "DE_BsMC_vs_FsMC.tsv"), row.names=1)
...
pheatmap(hm_scaled,
         ...
         show_colnames = FALSE,
         fontsize_row = 9,
         main = "Top 30 Tissue DE Genes (BsMC vs FsMC)\nZ-scored, batch-corrected logCPM")
dev.off()
...
tt_interaction <- topTable(fit_int, coef="TissueBreast:StateStimulated", number=Inf)
...
message("\n=== TOP INTERACTION GENES (different activation in BsMC vs FsMC) ===")
